...
import { useNavigate } from "react-router-dom";
import { User, Mail, Lock, Home } from "lucide-react";
import { useAuth } from "../contexts/AuthContext"; // 1. Import useAuth (check your path)

export default function Register() {
  const navigate = useNavigate();
  const { register } = useAuth(); // 2. Get the register function from context
  const [formData, setFormData] = useState({
    name: "",
    surname: "",
    email: "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Navigate to home, as register() logs the user in
      navigate("/"); 

    } catch (err: any) {
      // Handle errors from the api wrapper
